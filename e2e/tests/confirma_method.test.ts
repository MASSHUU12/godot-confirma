...
test("Empty '--confirma-run' and '--confirma-method', returns with error", async () => {
    ...
    expect(stderr.toString()).toContain(
        "Value for --confirma-method cannot be empty.",
    );
});

test("Empty '--confirma-run', returns with error", async () => {
    const { exitCode, stderr } = await runGodot(
        "--confirma-run",
        "--confirma-method=Lorem",
    );

    expect(exitCode).toBe(1);
    expect(stderr.toString()).toContain(
        "Invalid value: argument '--confirma-run' cannot be empty when using argument '--confirma-method'.",
    );
});
